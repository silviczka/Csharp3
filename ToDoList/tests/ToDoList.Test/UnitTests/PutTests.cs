...
{
    [Fact]
    public async Task Put_UpdateByIdWhenItemUpdated_CategoryProvided_Returns204NoContent()
    {
        ...
        repositoryMock.ReadByIdAsync(testId).Returns(existingToDoItem);
        repositoryMock.When(r => r.UpdateAsync(Arg.Any<ToDoItem>())).Do(x => { }); // Simulate successful update

        // Act
        var result = await controller.UpdateByIdAsync(testId, updateDto); // Call the method to update the item on position 1 with parameters of updateDto
    ...
    [Fact]
    public async Task Put_UpdateByIdWhenItemUpdated_CategorySetToNull_Returns204NoContent()
    {
        var repositoryMock = Substitute.For<IRepositoryAsync<ToDoItem>>();
        var controller = new ToDoItemsController(repositoryMock);
        int testId = 1;
    ...
    [Fact]
    public async Task Put_UpdateByIdWhenIdNotFound_Returns404NotFound()
    {
        // Arrange
        var repositoryMock = Substitute.For<IRepositoryAsync<ToDoItem>>();
        var controller = new ToDoItemsController(repositoryMock);
        ...
        repositoryMock.ReadByIdAsync(nonExistentId).Returns((ToDoItem)null);

        // Act
        var result = await controller.UpdateByIdAsync(nonExistentId, updateDto);

        // Assert
        Assert.IsType<NotFoundResult>(result); // Expecting 404 Not Found
        repositoryMock.Received(1).ReadByIdAsync(nonExistentId);
        repositoryMock.DidNotReceive().UpdateAsync(Arg.Any<ToDoItem>());
    }

    [Fact]
    public async Task Put_UpdateById_ReadByIdException_ReturnsInternalServerError()
    {
        // Arrange
        var repositoryMock = Substitute.For<IRepositoryAsync<ToDoItem>>();
        var controller = new ToDoItemsController(repositoryMock);
        ...
        repositoryMock.When(r => r.ReadByIdAsync(testId))
                    .Do(x => { throw new Exception("Unhandled exception in ReadById"); });

        // Act
        var result = await controller.UpdateByIdAsync(testId, updateDto);
    ...
    [Fact]
    public async Task Put_UpdateById_UpdateException_ReturnsInternalServerError()
    {
        // Arrange
        var repositoryMock = Substitute.For<IRepositoryAsync<ToDoItem>>();
        var controller = new ToDoItemsController(repositoryMock);
        ...
        repositoryMock.ReadByIdAsync(testId).Returns(new ToDoItem { ToDoItemId = testId, Name = "Old Name", Description = "Old Description", IsCompleted = false });

        // Set up Update to throw an exception
        repositoryMock.When(r => r.UpdateAsync(Arg.Any<ToDoItem>()))
